...
namespace InvestQ.Application.Services.Clientes
{
    ...
    {        
        ...
        public LancamentoService(ILancamentoRepo lancamentoRepo,
                                 ...
                                 ITaxaDeNegociacaoRepo taxaDeNegociacaoRepo,
                                 ITaxaDeIRRepo taxaDeIRRepo,
                                 IMapper mapper)
        {
        ...
        public async Task<LancamentoDto> AdicionarLancamento(LancamentoDto model)
        {
            ...
            if( await _lancamentoRepo.GetLancamentoByIdAsync(lancamento.Id, false, false) == null)
            {
                if (_tipoDeAtivo == TipoDeAtivo.Acao || 
                    _tipoDeAtivo == TipoDeAtivo.FundoImobiliario ||
                    _tipoDeAtivo == TipoDeAtivo.TesouroDireto)
                {
                    if ( lancamento.TipoDeMovimentacao == TipoDeMovimentacao.Compra || 
                         lancamento.TipoDeMovimentacao == TipoDeMovimentacao.Venda)
                    {
                        if ( lancamento.TipoDeMovimentacao == TipoDeMovimentacao.Venda) 
                        {
                            var taxaDeIR = await _taxaDeIRRepo
                                                    .GetTaxaDeIRByTipoDeAtivoAsync
                                                        (_tipoDeAtivo, lancamento.DataDaOperacao);

                            if (taxaDeIR != null)
                                lancamento.CustoIRDaOperacao = (lancamento.Quantidade * lancamento.ValorDaOperacao) * taxaDeIR.Percentual / 100;
                            else
                                lancamento.CustoIRDaOperacao = 0;
                        }

                        var taxaDeNegociacao = await _taxaDeNegociacaoRepo
                                                    .GetTaxaDeNegociacaoByTipoDeAtivoAsync
                                                        (_tipoDeAtivo, lancamento.DataDaOperacao);

                        if (taxaDeNegociacao != null)
                            lancamento.CustoB3DaOperacao = (lancamento.Quantidade * lancamento.ValorDaOperacao) * taxaDeNegociacao.Percentual / 100;
                        else
                            lancamento.CustoB3DaOperacao = 0;
                        ...
                        if (lancamentosDaCarteiraDoAtivoCompraVenda.Count() > 0 && _tipoDeAtivo != TipoDeAtivo.TesouroDireto) {
        ...
        public async Task<bool> DeletarLancamento(Guid lancamentoId)
        {
            ...
            if (_ativo.TipoDeAtivo == TipoDeAtivo.Acao || 
                _ativo.TipoDeAtivo == TipoDeAtivo.FundoImobiliario ||
                _ativo.TipoDeAtivo == TipoDeAtivo.TesouroDireto)
            {
                if ( lancamento.TipoDeMovimentacao == TipoDeMovimentacao.Compra || 
                     lancamento.TipoDeMovimentacao == TipoDeMovimentacao.Venda) 
                {
                                                              ...
                                                              , TipoDeAcaoDoUsuario tipoDeAcaoDoUsuario)
        {
            ...
            if (portifolio == null) {
                ...
                if (lancamentosDaCarteiraDoAtivo.Count() > 0) {
                    ...
                    if (tipoDeAcaoDoUsuario == TipoDeAcaoDoUsuario.Delete) 
                    {

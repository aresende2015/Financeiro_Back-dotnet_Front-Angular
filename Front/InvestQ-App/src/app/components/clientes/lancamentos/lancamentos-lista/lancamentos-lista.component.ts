...
import { TipoDeOperacao } from '@app/models/Enum/TipoDeOperacao.enum';
...
export class LancamentosListaComponent implements OnInit {
  ...
  function(_lancamento: Lancamento): number {
    var valorTotalDaOperacao: number = 0;
    if (_lancamento.tipoDeMovimentacao == TipoDeMovimentacao.Compra) {
      valorTotalDaOperacao = (_lancamento.quantidade * _lancamento.valorDaOperacao) +
                              _lancamento.custoB3DaOperacao;
    } else if(_lancamento.tipoDeMovimentacao == TipoDeMovimentacao.Venda) {
      valorTotalDaOperacao = (_lancamento.quantidade * _lancamento.valorDaOperacao) -
                              _lancamento.custoB3DaOperacao - _lancamento.custoIRDaOperacao;
    }

    return valorTotalDaOperacao;
  }
  ...
  public carregarLancamentos(): void {
    ...
    const observer = {
      next: (paginatedResult: PaginatedResult<Lancamento[]>) => {
        this.lancamentos = paginatedResult.result;
        this.lancamentos.forEach(x => x.valorTotalDaOperacao = this.function(x) );
        this.pagination = paginatedResult.pagination;
      },
      error: (error: any) => {
        this.spinner.hide();
